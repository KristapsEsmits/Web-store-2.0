    ...
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Brands
          <router-link class="btn btn-primary btn-round btn-fill float-end" to="/admin/brands/create">Add Brand
          </router-link>
        </h4>
      </div>
      <div class="card-body">
        <template v-if="brands.length > 0">
          <table class="table table-bordered">
            ...
            <tr v-for="(brand, index) in brands" :key="index">
              ...
              <td>{{ brand.img }}</td>
              <td class="image-cell">
                <img :src="'http://localhost:8000/storage/uploads/' + brand.img"
                     alt="Brand Image" style="max-width: 90px; max-height: 70px;">
              </td>
              <td class="d-flex justify-content-center">
                <router-link :to="{ path: '/admin/brands/' + brand.id + '/edit' }" class="btn btn-success float-middle">
                  Edit
                </router-link>
